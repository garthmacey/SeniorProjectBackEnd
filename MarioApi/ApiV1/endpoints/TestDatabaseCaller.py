...
class TestDatabaseCaller():
    ...
    def drive_types_insert(self, driveType):
        ...
        if str(driveType) in value:
            return False
        self.cursor.execute("Insert into testDrivetypes (driveType) values ('"
                            + str(driveType) + "');")
        self.cnxn.commit()
        return True
    ...
    def drive_types_update(self, oldDriveType, driveType):
        ...
        if not str(oldDriveType) in value or str(driveType) in value:
            return False
        self.cursor.execute("Update testDrivetypes set driveType = '"
    ...
    def drive_types_delete(self, driveType):
        ...
        try:
            if not self.valid_input(driveType):
                return False
            self.cursor.execute("delete from testDriveTypes where driveType = '" + str(driveType) + "';")
            self.cnxn.commit()
            return True
        except:
            return False
    ...
    def environment_insert(self, environment):
        ...
        if str(environment) in value:
            return False
        self.cursor.execute("Insert into testEnvironments (environment) values ('"
                            + str(environment) + "');")
        self.cnxn.commit()
        return True
    ...
    def environment_update(self, oldEnvironment, environment):
        ...
        if not str(oldEnvironment) in value or str(environment) in value:
            return False
        self.cursor.execute("Update testEnvironments set environment = '"
    ...
    def environment_delete(self, environment):
        ...
        try:
            if (not (self.valid_input(environment))):
                return False
            self.cursor.execute("delete from testEnvironments where environment = '" + str(environment) + "';")
            self.cnxn.commit()
            return True
        except:
            return False
    ...
    def buildIDs_patch(self, oldBuildID, buildID, driveType, environment):
        ...
        if str(buildID) in value or not str(oldBuildID) in value:
            return False
        self.cursor.execute("Update testBuildIDs Set buildID='" + str(buildID) + "', driveType='" + str(driveType)
    ...
    def buildIDs_delete(self, buildID):
        ...
        if not str(buildID) in value:
            return False
        self.cursor.execute("Delete from testBuildIDs where buildID='" + str(buildID) + "';")
        self.cnxn.commit()
        return True

    def buildIDs_getMaps(self):
        """Gets all build IDs with their associated driveType and EnvType"""
        self.cursor.execute("Select * from testBuildIDs;")
        values = []
        row = self.cursor.fetchone()
        row = self.cursor.fetchone()
        while row:
    ...
    def users_get(self, email):
        try:
            if (not(self.valid_input(email))):
                return False
            self.cursor.execute("Select descriptor from testUsers where email = '" + str(email) + "';")
            row = self.cursor.fetchone()
            return row[0]
        except:
            return False
    ...
    def users_insert(self, email, descriptor):
        try:
            if (not(self.valid_input(email) and self.valid_input(descriptor))):
                return False
            self.cursor.execute("Insert into testUsers(email, descriptor) values ('" + str(email) + "', '" + str(descriptor) + "');")
            self.cnxn.commit()
            return True
        except:
            return False
    ...
    def users_delete(self, email):
        try:
            if (not(self.valid_input(email))):
                return False
            self.cursor.execute("Delete from testUsers where email = '" + str(email) + "';")
            self.cnxn.commit()
            return True
        except:
            return False
    ...
    def users_update(self, email, descriptor):
        try:
            if (not(self.valid_input(email) and self.valid_input(descriptor))):
                return False
            self.cursor.execute("Update testUsers "
    ...
    def artifacts_insert(self, artifactID, artifactName, configID):
        try:
            if (not(self.valid_input(artifactID) and self.valid_input(artifactName) and self.valid_input(configID))):
                return False
            self.cursor.execute("Insert into testArtifacts(artifactID, artifactName, configID) values "
    ...
    def artifacts_get_all(self, configID):
        try:
            if (not(self.valid_input(configID))):
                return False
            self.cursor.execute("Select * from testArtifacts where configID = '" + str(configID) + "';")
            values = []
            row = self.cursor.fetchone()
            while row:
    ...
    def artifacts_delete(self, artifactID, configID):
        try:
            if (not(self.valid_input(artifactID) and self.valid_input(configID))):
                return False
            self.cursor.execute("Delete from testArtifacts where artifactID = '" + str(artifactID) + "' and configID = '" + str(configID) + "';")
            self.cnxn.commit()
            return True
        except:
            return False
    ...
    def artifacts_update(self, artifactID, artifactName, configID):
        try:
            if (not(self.valid_input(artifactID) and self.valid_input(artifactName) and self.valid_input(configID))):
                return False
            self.cursor.execute("Update testArtifacts "
