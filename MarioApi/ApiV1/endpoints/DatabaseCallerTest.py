...
class MyTestCase(unittest.TestCase):
    ...
    def test_users_get(self):
        received = dbCon.users_get("email@email.com")
        expected = "goodDescriptor"
        self.assertEqual(expected, received)

    def test_users_insert(self):
        received = dbCon.users_insert("testEmail@email.com", "testDescriptor")
        expected = True
        self.assertEqual(expected, received)

    def test_users_delete(self):
        received = dbCon.users_delete("email@email.com")
        expected = True
        self.assertEqual(expected, received)

    def test_users_update(self):
        received = dbCon.users_update("email@email.com", "newDescriptor")
        expected = True
        self.assertEqual(expected, received)

    def test_artifacts_insert(self):
        received = dbCon.artifacts_insert("art3", "Artifact 3", "TestConfig")
        expected = True
        self.assertEqual(expected, received)

    def test_artifacts_get_all(self):
        received = dbCon.artifacts_get_all("TestConfig")
        expected = [('art1', 'Artifact 1', 'TestConfig'), ('art2', 'Artifact 2', 'TestConfig')]
        self.assertEqual(expected, received)

    def test_artifacts_delete(self):
        received = dbCon.artifacts_delete("art1", "TestConfig")
        expected = True
        self.assertEqual(expected, received)

    def test_artifacts_update(self):
        received = dbCon.artifacts_update("art1", "New Name", "TestConfig")
        expected = True
        self.assertEqual(expected, received)
