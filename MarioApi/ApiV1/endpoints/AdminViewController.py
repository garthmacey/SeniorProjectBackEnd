    ...
    if check == 200:
        ...
        if request.method == "GET":
            ...
            if request.method == "POST":
                ...
                response = post_buildmaps(dbCaller, buildId, driveType, environment, nickname)
            elif request.method == "PATCH":
                ...
                environment = jsonbody["environment"]
                nickname = jsonbody["nickname"]
                response = patch_buildmaps(dbCaller, oldBuildId, buildId, driveType, environment, nickname)
            elif request.method == "DELETE":
                buildId = jsonbody["buildid"]
...
def get_buildmaps(dbCaller):
    ...
    for mappingRow in mappings:
        ...
        nickname = mappingRow[3]
        mappingList.append({"buildid": buildId, "environment": environment, "drivetype": driveType, "nickname" : nickname})
    data = {'buildmaps': mappingList}
    response = HttpResponse(json.dumps(data), content_type="application/json")
    return response


def post_buildmaps(dbCaller, buildId, driveType, environment, nickname):
    """Post Build Maps request handling method. Adds a new build mapping to the database with the provided"""
    """parameters"""
    if dbCaller.buildIDs_insert(buildId, driveType, environment, nickname):
        return HttpResponse(status=200)
    else:
        return HttpResponse(status=400)


def patch_buildmaps(dbCaller, oldBuildId, buildId, driveType, environment, nickname):
    """Patch Build Maps request handling method. Updates a build mapping with the provided old build id, with the"""
    """new provided params"""
    if dbCaller.buildIDs_patch(oldBuildId, buildId, driveType, environment, nickname):
        return HttpResponse(status=200)
    else:
        return HttpResponse(status=400)
